<template>
    <div id="login-panel" class="w-full dark:bg-black dark:text-white border md:min-w-[23rem] flex flex-col gap-2 p-5">
            ...
            <input type="email" autocomplete="email" :placeholder="$t('login.inputs.email_placeholder')" v-model="email" class="w-full max-w-full outline-none border p-2 dark:text-black">
            <input type="password" autocomplete="current-password" :placeholder="$t('login.inputs.password_placeholder')" v-model="password" class="outline-none border p-2 dark:text-black">
            <p v-if="message" class="text-red-500">{{ message }}</p>
            <button :disabled="$store.state.loading" @click="Login" class="bg-black flex justify-center items-center gap-2 text-white dark:bg-white dark:text-black font-bold py-2">
